...
DATA_MODEL_PATH=../EL.model.jsonld
DATA_MODEL=EL.model.jsonld
LOG_DIR=logs
...
for i in ${!CHANGED_TEMPLATES[@]}; do
    ...
    schematic manifest \
        --config schematic-config-test.yml \
        get -dt "${CHANGED_TEMPLATES[$i]}" \
        --title "${CHANGED_TEMPLATES[$i]}" \
        -s | tee $LOG_DIR/${CHANGED_TEMPLATES[$i]%.*}_log
